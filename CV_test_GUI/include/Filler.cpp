...
{
	// доступ к элементам массива осущетвл€етс€ в виде [x + y * filler_rows] эта запись эквивалаента записи [i][j]
	...
	if (x + 1 < filler_rows) // ѕроверка на выход за гранциы изображени€
	{
		fill_image_rec_ver(image_data, x + 1, y, pixel_brightness, filler_color); // вызываем рекрсивна функцию и так дл€ всех соседних пикселей с проверкой на выход за гранцы изображени€
	}
	if (x > 0)
	{
...
{
	std::queue<Pixel> queue; // ќб€вл€ем очередь
	queue.push(pos); // ƒобовл€ем в очередь начальную точку
	Pixel temp{ 0,0 }; // —оздаем временный обект дл€ хранени€ промежуточных точек
	while (!queue.empty()) // ѕока очередь не пуста
	{
		temp = queue.front(); // оброщ€емс€ к первому элементу очереди и сохрон€ем ее во временной переменной
		queue.pop();// удал€ем элемент из очереди

		if (temp.x + 1 < filler_rows)// ѕровер€ем границы изоброжени€
		{
			if (image_data[(temp.x + 1) + temp.y * filler_rows] <= pixel_brightness &&
				image_data[(temp.x + 1) + temp.y * filler_rows] != filler_color)// если наш соседниц <= пороговой €крокости и != цвету заливки тогда красим наш пиксель и
																				//добавл€ем в очередь кординаты соседнего пиксел€ и так дл€ всех соседних пикселей
			{
...
{
	auto rows = image.rows;				//  оличество строк в изоброжении
	auto columns = image.cols;			//  оличество столбцов в изоброжении
	auto image_data = image.data;		// ”казатель на основые данные изоброжени€ котора€ хранитс€ в виде массива байт
	std::queue<Pixel> queue;			// ќб€вл€ем очередь
	queue.push(pixel_pos);				// ƒобовл€ем в очередь начальную точку
	Pixel temp{ 0,0 };					// —оздаем временный обект дл€ хранени€ промежуточных точек
	while (!queue.empty())				// ѕока очередь не пуста
	{
		temp = queue.front();			// оброщ€емс€ к первому элементу очереди и сохрон€ем ее во временной переменной
		...
		if (temp.x + 1 < rows)			// ѕровер€ем границы изоброжени€
		{
			if (image_data[(temp.x + 1) + temp.y * rows] <= pixel_brightness &&
				image_data[(temp.x + 1) + temp.y * rows] != filler_color)// если наш соседниц <= пороговой €крокости и != цвету заливки тогда красим наш пиксель и
																		//добавл€ем в очередь кординаты соседнего пиксел€ и так дл€ всех соседних пикселей
			{
